...
<head>
    ...
    <title>Geen toegang — Proud Innovations</title>
...
<body class="font-sans antialiased bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="text-center max-w-md px-6">
        <div class="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6"
            ...
            <span class="text-white font-bold text-2xl">403</span>
        </div>
        <h1 class="text-xl font-semibold text-gray-800 mb-2">Geen toegang</h1>
        ...
        @auth
            @php
                $back = match(auth()->user()->role) {
                    'admin'            => route('beheer.dashboard'),
                    'werkvoorbereider' => route('werkbon.index'),
                    default            => route('verkoper.dashboard'),
                };
            @endphp
            <a href="{{ $back }}"
